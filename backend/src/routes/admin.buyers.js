...
router.post('/buyers', async (req,res)=>{
  ...
  const u = await User.create({ name, phone, email: (email||'').toLowerCase(), passwordHash, role:'buyer', addresses: address? [{ line:address }] : [] })
  res.json({ buyer: { id: u._id, name:u.name, email:u.email } })
})
router.put('/buyers/:id', async (req,res)=>{
  const patch = req.body
  if (patch.address){ patch.addresses = [{ line: patch.address }]; delete patch.address }
...
router.post('/buyers/:id/set-password', async (req,res)=>{
