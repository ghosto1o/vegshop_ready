...
router.post('/create-intent', async (req,res)=>{
  try{
    ...
    const srcResp = await fetch('https://api.omise.co/sources', {
      method:'POST',
      headers: { 'Authorization': omiseAuthHeader(), 'Content-Type':'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ type: method || 'internet_banking_bbl', amount: String(amount), currency:'thb' })
    })
    ...
    const chResp = await fetch('https://api.omise.co/charges', {
      method:'POST',
      headers: { 'Authorization': omiseAuthHeader(), 'Content-Type':'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ amount:String(amount), currency:'thb', source:src.id, return_uri:returnUri, 'metadata[orderId]': String(orderId) })
    })
...
router.post('/webhook', async (req,res)=>{
  try{
    ...
    if (secret && req.headers['x-webhook-secret'] !== secret) return res.status(401).json({ message:'unauthorized' })
    const evt = req.body
    const charge = evt?.data?.object
    if (charge?.object==='charge' && charge?.metadata?.orderId){
      const status = charge.paid ? 'paid' : (charge.status || 'processing')
      await Order.updateOne({ _id: charge.metadata.orderId }, { status, chargeId: charge.id })
    }
    res.json({ ok:true })
  }catch(e){
