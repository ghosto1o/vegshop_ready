...
router.put('/addresses/:index', async (req, res) => {
  ...
  const { line, phone, note } = req.body || {}
  const u = await User.findById(req.user._id)
  if (!u.addresses || idx < 0 || idx >= u.addresses.length)
    return res.status(404).json({ message: 'not found' })
  u.addresses[idx] = {
    line: line ?? u.addresses[idx].line,
    phone: phone ?? u.addresses[idx].phone,
    note: note ?? u.addresses[idx].note,
  }
  await u.save()
  res.json({ ok: true })
})
...
router.patch('/addresses/default', async (req, res) => {
  ...
  const u = await User.findById(req.user._id)
  if (!u.addresses || index < 0 || index >= u.addresses.length)
