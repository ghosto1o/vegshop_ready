...
export async function register(payload){
  ...
  const r = await fetch((import.meta.env.VITE_API_BASE || 'http://localhost:4000') + '/auth/register', {
    method:'POST',
  ...
  localStorage.setItem('veg_current_user', JSON.stringify(data.user))
  return data.user
}

export async function login(payload){
  console.log('[FE] login', payload.email)
  const r = await fetch((import.meta.env.VITE_API_BASE || 'http://localhost:4000') + '/auth/login', {
    method:'POST',
    headers: { 'Content-Type':'application/json' },
    credentials: 'include',
    body: JSON.stringify(payload)
...
export async function me(){
  try{
    ...
    localStorage.setItem('veg_current_user', JSON.stringify(r.user))
    return r.user
  }catch{
    return null
  }
