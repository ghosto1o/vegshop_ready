...
const initial = {
  products: [],
  cart: [],
  user: JSON.parse(localStorage.getItem('veg_current_user') || 'null'),
}
function reducer(s, a) {
  switch (a.type) {
    case 'SET_PRODUCTS':
      return { ...s, products: a.items }
    case 'ADD_TO_CART': {
      const ex = s.cart.find((x) => x.id === a.id)
      const cart = ex
        ? s.cart.map((x) => (x.id === a.id ? { ...x, qty: x.qty + 1 } : x))
    ...
    case 'UPDATE_QTY':
      return {
        ...s,
        cart: s.cart.map((x) => (x.id === a.id ? { ...x, qty: Math.max(1, a.qty) } : x)),
      }
    case 'REMOVE':
      return { ...s, cart: s.cart.filter((x) => x.id !== a.id) }
    case 'CLEAR_CART':
      return { ...s, cart: [] }
    default:
      return s
  ...
  return (
    ...
    >
      <div className="container" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ fontWeight: 800, letterSpacing: 0.2 }}>ผักหน้าบ้าน</div>
        ...
        <div className="nav" style={{ marginLeft: 16 }}>
          ...
          {isBuyer && (
            <Link to="/delivery" style={{ color: '#fff' }}>
              คำสั่งซื้อของฉัน
            </Link>
          )}
          {isAdmin && (
...
export default function VeggieShopMVP({ onOpenAuth }) {
  ...
  const map = useMemo(
    () => Object.fromEntries(s.products.map((p) => [p.id || p._id, { ...p, id: p.id || p._id }])),
    [s.products]
  )
  ...
  async function placeOrder({ deliveryMethod, address, payment }) {
    if (!isAuthed()) {
      console.log('[placeOrder] blocked: not authed')
    ...
    <div>
        ...
        >
          <div>
            ...
            <div style={{ color: '#166534' }}>
              ลดพิเศษ ผักใบเขียววันนี้ • ปลอดสาร • เก็บเช้า ขายเช้า
            </div>
          </div>
          <div
            style={{
            ...
            >
              ...
              <div style={{ padding: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span
                    className="badge"
                    style={{ background: '#e2f5ea', color: '#166534', border: '1px solid #bbf7d0' }}
                  >
                    {p.category}
                  </span>
                </div>
                ...
                >
                  <div>
                    <b style={{ fontSize: 16 }}>{currency(p.price)}</b>{' '}
                    <span style={{ fontSize: 12, color: '#666' }}>/ {p.unit}</span>
                  </div>
                  <button
                    className="btn"
                    disabled={p.stock <= 0}
                    onClick={() => {
                      console.log('[Shop] add to cart', p.id)
                      d({ type: 'ADD_TO_CART', id: p.id })
                    }}
                    style={{
      ...
      {showCart && (
        <div
          ...
          onClick={() => {
            console.log('[Cart] overlay click → close')
            setShowCart(false)
          }}
        >
          <aside
          ...
          >
            ...
            >
              <b style={{ letterSpacing: 0.2 }}>ตะกร้าของฉัน</b>
              <button
                className="btn"
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, marginTop: 6, alignItems: 'center' }}>
                        <button
                          className="btn"
                          onClick={() => d({ type: 'UPDATE_QTY', id: ci.id, qty: ci.qty - 1 })}
                        >
                          -
                        </button>
                        <input
                          ...
                          value={ci.qty}
                          onChange={(e) =>
                            d({ type: 'UPDATE_QTY', id: ci.id, qty: Number(e.target.value) })
                          }
                          min={1}
                        />
                        <button
                        ...
                        <button
                          className="btn"
                          onClick={() => {
                            console.log('[Cart] remove item', ci.id)
                            d({ type: 'REMOVE', id: ci.id })
                          }}
                        >
                          ลบ
                        </button>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <b style={{ letterSpacing: 0.2 }}>ทำการสั่งซื้อ</b>
          </div>
          <button
            className="btn"
            onClick={onClose}
        ...
        <div className="grid" style={{ gap: 12, padding: 14 }}>
          ...
          >
            {['จัดส่ง', 'ชำระเงิน', 'ยืนยัน'].map((t, i) => (
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {i < 2 && (
                  <div style={{ width: 28, height: 2, background: '#86efac', borderRadius: 2 }} />
                )}
              </div>
            ))}
          </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              >
                <input
                  ...
                  onChange={() => {
                    console.log('[Checkout] set delivery=pickup')
              ...
              >
                <input
                  ...
                  onChange={() => {
                    console.log('[Checkout] set delivery=delivery')
          ...
          <div>
            ...
            <select
              ...
              value={payment}
              onChange={(e) => {
                console.log('[Checkout] set payment', e.target.value)
            ...
            <div style={{ fontSize: 12, color: '#166534', marginTop: 6, opacity: 0.9 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button
              className="btn"
              onClick={() => {
                console.log('[Checkout] confirm order')
                onPlaceOrder({ deliveryMethod, address, payment })
              }}
              style={{
                background: '#16a34a',
                color: '#fff',
